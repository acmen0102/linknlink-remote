...
3. 可以查看以下信息：
   - **设备ID (Device ID)**: 启动时会以醒目的格式显示，用于标识当前设备
   - 连接状态
   - 错误信息
   - 代理配置信息
...
设备ID是系统自动生成的唯一标识符，用于设备注册和识别。您可以通过以下方式查看：

1. **在日志中查看**：启动 add-on 后，在日志标签页中会以醒目的格式显示设备ID
   ```
   ==========================================
...
2. **在配置文件中查看**：设备ID会自动保存到 `/config/device_id.txt` 文件中
   - 在 Home Assistant 中，可以通过文件编辑器或 SSH 访问此文件
   - 文件路径：`/config/addons_config/frpc/device_id.txt`（具体路径可能因系统而异）
